  ...
  <div class="case">
    ...
    <el-row>
      <el-col :sm="24" class="label">
        <div @click="caseDetail(item)" v-for="(item,index) in listData" :key="index" class="caseBox">
          <img :src="hostUrl+'/'+imgFilter(item.photoPath)">
          <div class="post">{{item.survey}}</div>
        </div>
...
export default {
  ...
  methods: {
    ...
    //跳转到专家详情页(即新闻详情页面)
    caseDetail: function(item) {
        this.$router.push({path:"/caseSummary",query:{id:item.id}})
    }
  }
};
</script>
