  ...
  <div>
    <el-row>
      <el-col :offset="1" :sm="22" class="trainAdd">
        <p class="title"><a onclick="window.history.go(-1)"><i class="el-icon-back"></i>返回</a> 添加课程培训</p>
        <div class="oprate">
        ...
        <el-form ref="trainData" :model="trainData" label-width="100px" :rules="rules">
          ...
          <el-form-item label="封面图：">
            ...
            <a class="delImg" v-show="trainData.coverImg!=''" @click="delImg()">删除图片</a>
          </el-form-item>
          ...
          <el-form-item label="添加课程" v-loading="courseLoading" element-loading-text="正在上传，请不要操作当前页面，以免文件丢失" element-loading-spinner="el-icon-loading" element-loading-background="rgba(226,226,226,0.2)">
...
export default {
  ...
  methods: {
    ...
    //删除上传的封面图
    delImg() {
      let param = { id: null, path: this.trainData.coverImg };
    ...
    delAtt: function(item) {
      ...
      if (data.backValue == true) {
        ...
        //this.attlist = removeAaary(this.trainData.atts, item);
        this.$message({
          type: "success",
    ...
    attUpload: function(atts, isMuiti, name, index) {
      ...
      if (name == "coverImg") {
        ...
        let summary=this.trainData.course[index].summary;
        data.attlist[0].summary=summary;
        console.info(data.attlist[0]);
    ...
    submit: function(status) {
      this.$refs["trainData"].validate(valid => {
        if (valid) {
          ...
          console.info(param);
          let data = postAjax(this.hostUrl + "/train/add", param);
          if (data.backValue == true) {
            this.$message({
              type: "success",
              message: "新增成功"
