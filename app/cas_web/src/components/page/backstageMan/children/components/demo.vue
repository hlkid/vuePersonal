...
export default {
  ...
  methods: {
    ...
    del: function(index, data) {
      let _this = this;
      this.$confirm(
        "此操作将永久删除”" + data[index].name + "“ 该课程, 是否继续?",
        "提示",
        {
        ...
        .then(async () => {
          ...
          $.ajax({
            url: _this.hostUrl + "/demoArea/delete",
            type: "post",
            dataType: "json",
    ...
    statusChange: function(index, data, val) {
      ...
      let _this = this;
      this.$confirm(
        "是否确定" + statusMsg + "”" + data[index].name + "“？",
        "提示",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
        ...
        .then(() => {
          ...
          param = JSON.stringify(param);
          $.ajax({
            url: _this.hostUrl + "/demoArea/update",
            type: "post",
            dataType: "json",
            contentType: "application/json;charset=utf-8",
            data: param,
    ...
    //新增
    add: function() {
      this.$router.push({ path: "/backstageMan/demo_add" });
    },
    //跳转修改页面
    edit: function(item) {
      this.$router.push({
