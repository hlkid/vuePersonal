...
export default {
  ...
  methods: {
    ...
    dataInit:function(){
      ...
      this.demoData.id = this.$route.query.id;
      let data = postAjax(this.hostUrl + "/demoArea/select", param).backValue;
      console.info(data);
      this.demoData.num=data.number;
      this.demoData.name=data.name;
      this.demoData.area=data.region;
      this.demoData.lv=data.grade;
      this.demoData.des=data.summary;
    },
    //提交信息
    submit: function(status) {
      this.$refs["demoData"].validate(valid => {
        if (valid) {
          ...
          console.info(param);
          let data = postAjax(this.hostUrl + "/demoArea/update", param);
