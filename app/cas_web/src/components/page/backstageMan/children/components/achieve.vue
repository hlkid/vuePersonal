        ...
        <el-row class="search">
          ...
          <el-col :sm="2" class="status">
            <el-select v-model="search.status" placeholder="请选择" clearable>
              ...
              <el-option key="2" label="停用" value="3"> </el-option>
            </el-select>
          </el-col>
...
export default {
  ...
  methods: {
    ...
    del: function(index, data) {
      let _this = this;
      this.$confirm(
        "此操作将永久删除”" + data[index].name + "“该信息, 是否继续?",
        "提示",
        {
        ...
        .then(async () => {
          ...
          $.ajax({
            url: _this.hostUrl + "/resultsShow/delete",
            type: "post",
            dataType: "json",
    ...
    statusChange: function(index, data, val) {
      ...
      let _this = this;
      this.$confirm(
        "是否确定" + statusMsg + "”" + data[index].name + "“ 该课程？",
        "提示",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
        ...
        .then(() => {
          ...
          param = JSON.stringify(param);
         $.ajax({
            url: _this.hostUrl + "/resultsShow/update",
            type: "post",
            dataType: "json",
            contentType: "application/json;charset=utf-8",
            data: param,
    ...
    //新增
    add: function() {
      this.$router.push({ path: "/backstageMan/achieve_add" });
    },
    //跳转修改页面
    edit: function(item) {
      this.$router.push({ path: "/backstageMan/achieve_edit",query:{id:item.id} });
    },
    //栏目节点点击事件
